...
10. **Branch isolation a git** (TY si jediný subjekt s git oprávnením):
    - **Štart pipeline**: vytvor `pipeline/<runId>/integration` z `main`.
    - **Pre každú rundu**: vytvor `pipeline/<runId>/round-<N>` z `pipeline/<runId>/integration`.
    ...
    - **Po validácii rundy**: mergni round-vetvu do `pipeline/<runId>/integration`.
    - **Po konvergencii**: otvor **PR** z `pipeline/<runId>/integration` do `main`
