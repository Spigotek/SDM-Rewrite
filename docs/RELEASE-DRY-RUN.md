...
| B2  | CA SDM dev backend `10.11.35.35:8050` is **TCP-unreachable** from `10.11.36.21`. Both hosts share the `10.11.32.0/19` supernet (routing OK), so the denial is an intra-/19 rule — host firewall on `10.11.35.35`, VLAN isolation on the L2 fabric, or IPS policy. `nc/curl` time out from host shell and BFF container alike. | 🔴 Operator-side. Open ticket with the network team to allow `10.11.36.21 → 10.11.35.35:8050` (TCP). |
