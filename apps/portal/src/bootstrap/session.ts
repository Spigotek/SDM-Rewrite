 ...
 * happy path strips suspended tenants server-side, but a status field is
 * still threaded through so admin / SP cockpit views (post-MVP) can render
 * the suspended state with a tooltip instead of removing the entry.
 */
 ...
 * is suspended. Distinct from `UnauthorizedError` (401) and a generic 403 — the
 * SessionContext maps this specifically to a toast + a forced sign-out flow.
 */
...
export async function postActiveTenant(tenantId: TenantId): Promise<SessionLoadResult> {
  ...
  // I.3 — 403 with `details.reason: "tenant_suspended"` is its own
  // failure mode: the FE drops to anonymous + surfaces a toast, then the
  // user re-authenticates (no in-place recovery — the suspended tenant
  // remains denied even on the next session until admin restores it).
