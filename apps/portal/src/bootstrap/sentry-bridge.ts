 ...
 * wires React Router + TanStack Query the eager-load budget (180 KB portal)
 * is exhausted, so this bridge defers the entire Sentry graph behind a
 * dynamic import scheduled in `requestIdleCallback`.
 *
 ...
 * `<ErrorBoundary>` fallback so the user still sees a useful screen. This
 * matches `audit-and-compliance.md §3 sampling` philosophy.
 *
 * Usage:
 *   await captureException(err);           // lazy — fires once SDK is loaded
 *   await setSentryUser({ id: pseudId });  // queued until ready
 *
 * Anything that needs Sentry synchronously (e.g. a custom ErrorBoundary
 * fallback render) should fall back gracefully when `isSentryReady()` is
 * still false on first call.
 */
...
/**
 * Schedule Sentry init for the next idle frame. Safe to call before the
 * runtime config is available — if `dsn` is missing the bridge no-ops.
 */
