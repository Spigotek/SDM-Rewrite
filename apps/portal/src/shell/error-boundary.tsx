 ...
 * footprint would push the initial JS over the portal budget post-H.0
 * (per G.4 deferred trade-off). Instead, we use a plain React class boundary
 * and forward the error to the lazy `sentry-bridge` which dynamically loads
 * the SDK on idle. Errors caught before Sentry is ready are queued and
 * drained once the bridge initialises.
 */
