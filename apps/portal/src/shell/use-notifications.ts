/**
 * L.1.B — Notification queue for the portal top-bar bell.
 *
...
const STORAGE_KEY = "sdm.portal.notifications.lastReadAt";
const QUEUE_MAX = 50;
...
/**
 * Map a raw J.3 event to a renderer-friendly notification. Returns null for
 * `connected` (informational only) so the bell never lights up on reconnect.
 */
function mapEvent(
  event: AppEvent,
  t: (key: string) => string,
...
export function useNotifications(): UseNotificationsResult {
  ...
  useEffect(() => {
    return subscribe((event) => {
      ...
      if (!mapped) return;
      setEvents((prev) => {
        // Most-recent first, capped at QUEUE_MAX.
