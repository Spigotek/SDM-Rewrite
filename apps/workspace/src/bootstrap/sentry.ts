 ...
 * `Sentry.init()` synchronously before React render. After H.0 the
 * workspace's eager-load JS budget can't accommodate the 41 KB Sentry chunk
 * on the critical path, so init is scheduled in `requestIdleCallback` via
 * `scheduleSentryInit()` in `sentry-bridge.ts`.
 *
 ...
 * renders a fallback so users see something useful.
 */
